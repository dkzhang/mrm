...
type MeetingQuery struct {
	config
	ctx        *QueryContext
	order      []meeting.OrderOption
	inters     []Interceptor
	predicates []predicate.Meeting
	withMdrs   *MeetingDateRoomQuery
...
func (mq *MeetingQuery) Order(o ...meeting.OrderOption) *MeetingQuery {
	mq.order = append(mq.order, o...)
	return mq
}

// QueryMdrs chains the current query on the "mdrs" edge.
func (mq *MeetingQuery) QueryMdrs() *MeetingDateRoomQuery {
	query := (&MeetingDateRoomClient{config: mq.config}).Query()
...
func (mq *MeetingQuery) All(ctx context.Context) ([]*Meeting, error) {
	...
	qr := querierAll[[]*Meeting, *MeetingQuery]()
	return withInterceptors[[]*Meeting](ctx, mq, qr, mq.inters)
}
...
func (mq *MeetingQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, mq, querierCount[*MeetingQuery](), mq.inters)
}
...
func (mq *MeetingQuery) Clone() *MeetingQuery {
	...
	return &MeetingQuery{
		config:     mq.config,
		ctx:        mq.ctx.Clone(),
		order:      append([]meeting.OrderOption{}, mq.order...),
		inters:     append([]Interceptor{}, mq.inters...),
		predicates: append([]predicate.Meeting{}, mq.predicates...),
		withMdrs:   mq.withMdrs.Clone(),
...
// WithMdrs tells the query-builder to eager-load the nodes that are connected to
// the "mdrs" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MeetingQuery) WithMdrs(opts ...func(*MeetingDateRoomQuery)) *MeetingQuery {
	query := (&MeetingDateRoomClient{config: mq.config}).Query()
...
//
//	client.Meeting.Query().
//		Select(meeting.FieldName).
//		Scan(ctx, &v)
func (mq *MeetingQuery) Select(fields ...string) *MeetingSelect {
	mq.ctx.Fields = append(mq.ctx.Fields, fields...)
	sbuild := &MeetingSelect{MeetingQuery: mq}
	sbuild.label = meeting.Label
...
func (mq *MeetingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Meeting, error) {
	...
	if query := mq.withMdrs; query != nil {
		if err := mq.loadMdrs(ctx, query, nodes,
			func(n *Meeting) { n.Edges.Mdrs = []*MeetingDateRoom{} },
			func(n *Meeting, e *MeetingDateRoom) { n.Edges.Mdrs = append(n.Edges.Mdrs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MeetingQuery) loadMdrs(ctx context.Context, query *MeetingDateRoomQuery, nodes []*Meeting, init func(*Meeting), assign func(*Meeting, *MeetingDateRoom)) error {
	...
	query.withFKs = true
	query.Where(predicate.MeetingDateRoom(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(meeting.MdrsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.meeting_mdrs
		if fk == nil {
			return fmt.Errorf(`foreign-key "meeting_mdrs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "meeting_mdrs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (mgb *MeetingGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*MeetingQuery, *MeetingGroupBy](ctx, mgb.build, mgb, mgb.build.inters, v)
}

func (mgb *MeetingGroupBy) sqlScan(ctx context.Context, root *MeetingQuery, v any) error {
...
func (ms *MeetingSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*MeetingQuery, *MeetingSelect](ctx, ms.MeetingQuery, ms, ms.inters, v)
}

func (ms *MeetingSelect) sqlScan(ctx context.Context, root *MeetingQuery, v any) error {
