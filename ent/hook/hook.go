...
// The MeetingDateRoomFunc type is an adapter to allow the use of ordinary
// function as MeetingDateRoom mutator.
type MeetingDateRoomFunc func(context.Context, *ent.MeetingDateRoomMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MeetingDateRoomFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MeetingDateRoomMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MeetingDateRoomMutation", m)
}
