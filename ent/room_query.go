...
func (rq *RoomQuery) QueryMdrs() *MeetingDateRoomQuery {
	query := (&MeetingDateRoomClient{config: rq.config}).Query()
...
func (rq *RoomQuery) WithMdrs(opts ...func(*MeetingDateRoomQuery)) *RoomQuery {
	query := (&MeetingDateRoomClient{config: rq.config}).Query()
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withMdrs; query != nil {
		if err := rq.loadMdrs(ctx, query, nodes,
			func(n *Room) { n.Edges.Mdrs = []*MeetingDateRoom{} },
			func(n *Room, e *MeetingDateRoom) { n.Edges.Mdrs = append(n.Edges.Mdrs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadMdrs(ctx context.Context, query *MeetingDateRoomQuery, nodes []*Room, init func(*Room), assign func(*Room, *MeetingDateRoom)) error {
	...
	query.Where(predicate.MeetingDateRoom(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.MdrsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.room_mdrs
		if fk == nil {
			return fmt.Errorf(`foreign-key "room_mdrs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "room_mdrs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
